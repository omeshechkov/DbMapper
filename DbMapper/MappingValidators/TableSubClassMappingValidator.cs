    ...
    public sealed class TableSubClassMappingValidator : MappingValidator
    {
        ...
        public override void Validate(object mapping, object context)
        {
            ...
            if (subClassMapping.Type == null)
                throw new ValidationException("Table subclass mapping validation error, type is null");

            var parent = subClassMapping.Parent;
            if (parent == null)
                throw new ValidationException("Table subclass mapping '{0}' validation error, parent class is null", subClassMapping.Type.AssemblyQualifiedName);

            if (subClassMapping.Properties == null)
                throw new ValidationException("Table subclass mapping '{0}' validation error, properties is null", subClassMapping.Type.AssemblyQualifiedName);

            if (!parent.Type.IsAssignableFrom(subClassMapping.Type))
                throw new ValidationException("Table subclass mapping '{0}' validation error, class '{0}' is not inherited from '{1}'",
            ...
            try
            {
                ...
                if (discriminator != null)
                {
                    if (subClassMapping.Type.IsAbstract)
                    {
                        ...
                        {
                            throw new ValidationException("Table subclass mapping '{0}' validation error, abstract class cannot have discriminator-value",
                    ...
                    else
                    {
                        if (subClassMapping.DiscriminatorValue == null)
                        {
                            throw new ValidationException("Table subclass mapping '{0}' validation error, non abstact class with discriminator column should have discriminator-value",
                                subClassMapping.Type.AssemblyQualifiedName);
                        }
                        ...
                        if (subClassMapping.DiscriminatorValue.GetType() != discriminator.Type)
                        {
                            throw new ValidationException(
                                "Table subclass mapping '{0}' validation error, discriminator value type is not match discriminator column type, expected: '{1}', actual: '{2}'",
