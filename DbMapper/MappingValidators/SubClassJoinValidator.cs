...
namespace DbMapper.MappingValidators
{
    ...
    public sealed class SubClassJoinValidator : MappingValidator
    {
        ...
        {
            if (mapping == null)
                throw new ValidationException("Subclass join mapping validation error, mapping is null");

            var subClassJoin = mapping as ISubClassJoin;
            if (subClassJoin == null)
                throw new ValidationException("Subclass join mapping validation error, mapping '{0}' is not a subclass join mapping", mapping.GetType().AssemblyQualifiedName);

            if (string.IsNullOrEmpty(subClassJoin.Table))
                throw new ValidationException("Subclass join mapping validation error, table name is null or empty");

            if (subClassJoin.ColumnJoins == null)
                throw new ValidationException("Subclass join mapping validation error, columns is null");

            if (subClassJoin.ColumnJoins.Count == 0)
                throw new ValidationException("Subclass join mapping validation error, columns is empty");
